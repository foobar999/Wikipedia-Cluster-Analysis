...
( time python3 scripts/bow_to_topic.py --bow=$BOW_MODEL.bz2 --id2word=$BOW_ID2WORD.bz2 --model-type=$TOPIC_MODEL --model-prefix=$MODEL_PREFIX --num-topics=$NUM_TOPICS --passes=$PASSES --iterations=$ITERATIONS --alpha=$ALPHA --beta=$BETA ) |& tee $LOG_TOPIC
# TODO .bz2 oder nicht???

echo "generating dense document-topic-file"
( time python3 scripts/utils/get_dense_document_topics.py --bow=$BOW_MODEL.bz2 --model-prefix=$MODEL_PREFIX --document-topics=$DOC_TOPICS_FILE ) |& tee -a $LOG_TOPIC
