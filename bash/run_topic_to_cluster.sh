...
(time python scripts/topic_to_cluster.py --document-topics=$DOCUMENT_TOPICS --cluster-labels=$CLUSTER_LABELS --cluster-method=$METHOD --num-clusters=$NUM_CLUSTERS) |& tee $LOG_CLUSTER
bzip2 -zf $CLUSTER_LABELS

echo "evaluating clustering"
(time python scripts/evaluate_dense_clustering.py --document-topics=$DOCUMENT_TOPICS --cluster-labels=$CLUSTER_LABELS.bz2) |& tee -a $LOG_CLUSTER
