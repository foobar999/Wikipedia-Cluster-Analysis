...
( time python scripts/accumulate_contribs.py --raw-contribs=$CONTRIB_PREFIX-raw-contributions.mm.bz2 --acc-contribs=$CONTRIB_PREFIX-doc-auth-contribs.mm ) |& tee $LOG_PREFIX-doc-auth-contribs.log

echo "transforming (docid,authorid,contribvalue) file to (authorid,docid,contribvalue) file"
( time ./bash/get_swapped_author_doc_contribs.sh $CONTRIB_PREFIX-doc-auth-contribs.mm > $CONTRIB_PREFIX-auth-doc-contribs.mm) |& tee $LOG_PREFIX-auth-doc-contribs.log
...
( time python scripts/contribs_to_graph.py --contribs=$CONTRIB_PREFIX-auth-doc-contribs.mm.bz2 --graph=$GRAPH_PREFIX-co-authorship.cpickle.bz2 ) |& tee $LOG_PREFIX-graph.log
