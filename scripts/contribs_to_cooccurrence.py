...
def main():
    parser = argparse.ArgumentParser(description='maps a given contribution file [(x1,y1,wa),(x1,y2,wb),...,(x2,y1,wc),...] to a co-occurence adjacency list [(y1,y2,v1),(y1,y3,v2),...]; must be sorted by x,y; w-values are conjuncted by minimum', epilog='Example: ./{} --contribs=enwiki-auth-doc-contribs.mm.bz2 --graph=enwiki-document-graph.cpickle '.format(sys.argv[0]))
    ...
    parser.add_argument('--co-occurence', type=argparse.FileType('w'), help='path to output co-occurence .mm MatrixMarket file', required=True)
    
    args = parser.parse_args()
    input_contribs_path = args.contribs.name
    output_co_occurence_path = args.co_occurence.name
    
    logger.info('running {} with:\n{}'.format(program, pformat({'input_contribs_path':input_contribs_path, 'output_co_occurence_path':output_co_occurence_path})))
