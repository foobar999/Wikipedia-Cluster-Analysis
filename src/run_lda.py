...
def main():
    ...
    #lda_model = LdaMulticore(corpus=mmcorpus, num_topics=numtopics, id2word=dictionary, passes=passes, iterations=iterations, chunksize=2000, alpha='symmetric', eval_every=1)
    
    from gensim.models import LdaModel # TODO produktiv durch LdaMulticore ersetzen
    lda_model = LdaModel(corpus=mmcorpus, num_topics=numtopics, id2word=dictionary, passes=passes, iterations=iterations, chunksize=2000, alpha='symmetric', eval_every=1)
